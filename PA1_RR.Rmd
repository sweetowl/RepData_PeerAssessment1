...
Mean of the total number of stpes taken per day amounts to `r mean(steps_per_day$steps)`.

```{r results="hide"}
mean(steps_per_day$steps)
```

Median for the total number of stpes taken per day is `r median(steps_per_day$steps)`.
...
The maximum number of steps is taken in the `r steps_interval$interval[which.max(steps_interval$steps)]` 5-minute interval.
...
Total number of rows with missing values amounts to `r sum(is.na(activity))`.
```{r results="hide"}
sum(is.na(activity))
```
...
Mean of the total number of stpes taken per day amounts to `r mean(steps_per_day_wo_NAs$steps)`.

```{r results="hide"}
mean(steps_per_day_wo_NAs$steps)
```

Median for the total number of stpes taken per day is `r median(steps_per_day_wo_NAs$steps)`.
